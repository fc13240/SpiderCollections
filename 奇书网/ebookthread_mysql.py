...
class qishuebook(object):
    ...
    def ebookcontent(self):
        '''使用多线程对每个分类'''
        th = []
        for target in self.savestarturl[1:]:
            classify = list(target.keys())[0]
            url = list(target.values())[0]
            t = Thread(target=self.getcontent, args=(classify, url))
            t.start()
            th.append(t)
    ...
    def getcontent(self, classify,url):
        ...
        while page <= number:
            ...
            for i in range(math.ceil(len(contents)/2)):
                detailth = []
                for content in contents[i*2:(i+1)*2]:
                    detailurl = self.start_url + self.split_content(content.xpath('./a/@href'))
                    dt = Thread(target=self.ebookdetail, args=(classify,url,detailurl))
                    dt.start()
                    detailth.append(dt)
    ...
    def ebookdetail(self,classify,url,detailurl):
        '''进入到书籍详情链接后获取详情信息'''
        pagedetail = requests.get(detailurl, headers=self.headers, proxies=self.proxies, verify=False).content
        htmldetail = etree.HTML(pagedetail)
